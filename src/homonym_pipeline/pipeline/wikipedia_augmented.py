...
def run_wikipedia_augmented(input_path: str | Path, output_dir: str | Path, config: AppConfig,
    ...
    original_lemma_count = len(lemmas)
    if max_lemmas:
        lemmas = lemmas[:max_lemmas]
    input_metadata = {
        "run_id": run_id,
        "original_lemmas": original_lemma_count,
        "single_gloss_lemmas_removed": 0,
        "lemmas_after_max_lemmas": len(lemmas),
        "requested_max_lemmas": max_lemmas,
    }
    atomic_write_json(root / "run_input_statistics.json", input_metadata)
    ...
    for index, lemma in enumerate(lemmas, start=1):
        ...
        wikipedia_started = time.perf_counter()
        wikipedia_elapsed_seconds = 0.0
        gloss_augmentation_elapsed_seconds = 0.0
        wikipedia_cache_hit = False
        terra_cache_hit = False
        candidates = []
        ...
        try:
            ...
            wikipedia_cache_hit = raw_record is not None
            if raw_record:
            ...
            wikipedia_elapsed_seconds = time.perf_counter() - wikipedia_started
            terra_started = time.perf_counter()
            ...
            terra_cache_hit = cached is not None
            if cached:
                ...
                call_payload.update({
                    ...
                    "stage": "wikipedia_gloss_augmentation",
                    "lemma": lemma,
                })
                normalized_record = {"cache_key": norm_key, "lemma": lemma,
                                     ...
                                     "llm_call": call_payload}
                append_jsonl(normalized_path, normalized_record)
                normalized_cache[norm_key] = normalized_record
                append_jsonl(calls_path, {"cache_key": norm_key, **call_payload})
            gloss_augmentation_elapsed_seconds = time.perf_counter() - terra_started
            ...
            stage_metadata[lemma] = {
                ...
                "terra_actions": terra_actions,
                "wikipedia_elapsed_seconds": wikipedia_elapsed_seconds,
                "gloss_augmentation_elapsed_seconds": gloss_augmentation_elapsed_seconds,
                "wikipedia_cache_hit": wikipedia_cache_hit,
                "terra_cache_hit": terra_cache_hit,
            }
            ...
            logger.info(
                ...
                "  Wikipedia retrieval time: %.2f seconds\n"
                "  Gloss augmentation time: %.2f seconds\n"
                ...
                len(candidates),
                len(glosses),
                wikipedia_elapsed_seconds,
                gloss_augmentation_elapsed_seconds,
                time.perf_counter() - lemma_started,
            )
            ...
            failed_audit = LemmaAuditRecord(
                ...
                wikipedia_elapsed_seconds=round(wikipedia_elapsed_seconds, 6),
                gloss_augmentation_elapsed_seconds=round(gloss_augmentation_elapsed_seconds, 6),
                wikipedia_cache_hit=wikipedia_cache_hit,
                terra_cache_hit=terra_cache_hit,
                wikipedia_candidates=len(candidates),
                terra_actions=terra_actions,
    ...
    input_metadata["glosses_after_wikipedia_augmentation"] = sum(len(entry.glosses) for entry in entries)
    atomic_write_json(root / "run_input_statistics.json", input_metadata)
